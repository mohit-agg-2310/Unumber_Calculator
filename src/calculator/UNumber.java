...
UNumber implements Comparable<UNumber> {
	...
	private byte d [];		// The mantissa as a sequence of digits, most significant digits (MSD)
	 ...
	 * 
	 * @param v  This parameter length will be counted
	 */
	 ...
	 * 
	 * @param v   This parameter will create a variable length number
	 */
	...
	public int getCharacteristic( double xy)
	{
	 ...
	 * 
	 * @param that   This parameter is a UNumber type variable that is used as first operand 
	 */
	 ...
	 * 
	 * @param that   This is a UNumber type variable
	 * 
	 * @param size     This parameter specifies the size of the other parameter
	 */
	 ...
	 * 
	 * @param v    This parameter will be converted to UNumber
	...
	public boolean compareUNumber(UNumber x, UNumber y)
	{
		int j=0;
		
		String sr1 = x.toDecimalString();
		String sr2 = y.toDecimalString();
		while(j <25)
		{
		if (sr1.charAt(j)!=sr2.charAt(j))
			break;
		j++;
	}
		if(j==24)	
	...
	public UNumber squareRoot(UNumber originalVal)
	{
		
		UNumber temp = new UNumber(originalVal);		
		UNumber y = new UNumber(2);
		UNumber w;  //= new UNumber();
		int i=0;
		do
		{
			UNumber x = new UNumber(originalVal);
			UNumber z= new UNumber(temp);
			w = new UNumber(temp);
			x.div(z);
			temp.add(x);
			temp.div(y);
			i++;
			System.out.println("The result after "+i+" iteration is: " + temp.toDecimalString());
		}while(i<8);                            
		return temp;                            
	}
}
